...
## Reads
The data we are using for this workshop are reads selected from a ~27M region in the kiwifruit genome. Assembling a full genome will take too long and a laptop is not a suitable device to assembly such a large genome. There are four read files:
~~~
